...
def gp_sampler(
    ...
    """ 
        ...
        \textbf{K} = \text{kernel\_se}(\textbf{x}, \textbf{x}, \text{hyp})
        \textbf{K}_{\text{chol}} = \text{Cholesky}(\textbf{K} + \sigma_n^2 \textbf{I})
        \textbf{f} = \textbf{K}_{\text{chol}} \cdot \textbf{z}
    """
...
def hbm_sampler(
    ...
    """
        ...
        \textbf{K} = \text{kernel\_se}(\textbf{x}, \textbf{x}, \text{hyp})
        \textbf{U}, \boldsymbol{\Lambda} = \text{eigen-decomposition}(\textbf{K})
        \textbf{f} = \textbf{U} \cdot \sqrt{\boldsymbol{\Lambda}} \cdot \textbf{z}
    """
    if len(times.shape) == 1: times = times.reshape((-1,1))
    L = times.shape[0]
    K = kernel_se(times,times,hyp={'gain':hyp_gain,'len':hyp_len}) # [L x L]
