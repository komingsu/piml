...
from models.mlp import MultiLayerPerceptronClass as MLP
...
class LinearAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # output projection
        self.proj = zero_module(nn.Linear(config.n_embd, config.n_embd))
    ...
    def forward(self, x, y=None, layer_past=None):
        ...
        attention_output = self.attn_drop((q @ context) * D_inv + q)

        # output projection
        projected_output = rearrange(attention_output, 'b h n d -> b n (h d)')
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, x, y):
        x = self.crossattn(self.ln1(x), self.ln_branchs(y))
        x = x + self.mlp1(self.ln3(x))
        x = self.selfattn(self.ln4(x))
